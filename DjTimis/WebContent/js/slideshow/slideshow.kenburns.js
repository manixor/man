	...
	Slideshow.KenBurns = new Class(
			{
				...
				_show : function(fast) {
					...
					var props = [ "top left", "top right", "bottom left",
							"bottom right" ][this.counter % 4].split(" ");
					this.image.setStyles([ 0, 0 ].associate(props));
					var src = this.data.images[this._slide].replace(
							/([^?]+).*/, "$1"), cache = this.cache[src];
					dh = this.height / cache.height;
					dw = this.width / cache.width;
					delta = dw > dh ? dw : dh;
					var values = {}, zoom = Number.random.apply(Number,
							this.options.zoom) / 100 + 1, pan = Math
							.abs(Number.random.apply(Number, this.options.pan) / 100 - 1);
					[ "height", "width" ].each(function(prop, i) {
						var e = Math.ceil(cache[prop] * delta), s = (e * zoom)
								.toInt();
						values[prop] = [ s, e ];
						if (dw > dh || i) {
