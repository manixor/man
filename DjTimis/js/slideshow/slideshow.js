	...
	Slideshow = new Class(
			{
				...
				initialize : function(el, data, options) {
					...
					this.slide = this._slide = this.options.match && match ? match[1]
							.toInt()
							: this.options.slide;
					...
					var keys = "slideshow first prev play pause next last images captions controller thumbnails hidden visible inactive active loader"
							.split(" "), values = keys.map(function(key, i) {
					...
					if (!data) {
						this.options.hu = "";
						data = {};
						var thumbnails = this.el.getElements(this.classes
								.get("thumbnails")
								+ " img");
						this.el
								.getElements(
										this.classes.get("images") + " img")
								.each(
										function(img, i) {
											var src = img.src, caption = img.alt
													|| img.title, href = img
													.getParent().href, thumbnail = thumbnails[i] ? thumbnails[i].src
													: "";
											data[src] = {
												caption : caption,
												href : href,
					...
					this.events.push("keyup", (function(e) {
						Object.each(this.accesskeys,
								function(accesskey, action) {
									if (e.key == accesskey.key
											&& e.shift == accesskey.shift
											&& e.control == accesskey.control
											&& e.alt == accesskey.alt) {
										this[action]();
									}
								}, this);
					}).bind(this));
					var el = this.el.getElement(this.classes.get("images")), img = this.el
							.getElement("img")
							|| new Element("img"), images = el ? el.empty()
							: (new Element("div", {
								'class' : this.classes.get("images").substr(1)
							})).inject(this.el), div = images.getSize();
					...
					[ this.a, this.b ].each(function(img) {
						anchor.clone().cloneEvents(anchor).grab(img).inject(
								images);
					});
				...
				go : function(n, direction) {
					...
					this.direction = direction ? direction
							: n < this._slide ? "right" : "left";
					this.slide = this._slide = n;
					if (this.preloader) {
						this.preloader = this.preloader.destroy();
					}
					this._preload(this.options.fast & WhenPlaying
							|| this.paused && this.options.fast & WhenPaused);
				},
				first : function() {
					this.prev(true);
				},
				...
				pause : function(p) {
					...
					if (this.paused) {
						...
						this.controller
								&& this.el.retrieve("pause").getParent()
										.removeClass(this.classes.play);
					} else {
						this.paused = true;
				...
				load : function(data) {
					...
					for ( var image in data) {
						var obj = data[image] || {}, image = this.options.hu
								+ image, caption = obj.caption ? obj.caption
								.trim() : "", href = obj.href ? obj.href.trim()
								: this.options.linked ? image
										: this.options.href, target = obj.target ? obj.target
								.trim()
								: "_self", thumbnail = obj.thumbnail ? this.options.hu
								+ obj.thumbnail.trim()
								: image.replace(this.options.replace[0],
										this.options.replace[1]), title = caption
					...
					if (this.options.random) {
						this.slide = this._slide = Number.random(0,
				...
				destroy : function(p) {
					...
					this.pause(1);
					"caption loader thumbnails".split(" ").each(
							function(i, timer) {
								this.options[i]
										&& (timer = this[i].retrieve("timer"))
										&& clearTimeout(timer);
							}, this);
				...
				_preload : function(fast) {
					var src = this.data.images[this._slide].replace(
							/([^?]+).*/, "$1"), cached = loaded = !!this.cache[src];
					if (!cached) {
						if (!this.preloader) {
				...
				_show : function(fast) {
					...
					var hidden = this.classes.get("images",
							this.direction == "left" ? "next" : "prev"), visible = this.classes
							.get("images", "visible"), img = this.counter % 2 ? this.a
							: this.b;
					if (fast) {
						...
						if (this.options.overlap) {
							...
							hidden = this.classes.get("images",
									this.direction == "left" ? "prev" : "next");
							this.image.get("morph").set(hidden);
							img.get("morph").set(visible).start(hidden).chain(
									fn);
						}
					}
				},
				_loaded : function(fast) {
					this.counter++;
					this.timeToNextTransition = Date.now()
							+ this.options.duration + this.options.delay;
					this.direction = "left";
					this.timeToTransitionComplete = fast ? 0 : Date.now()
				...
				_resize : function(img) {
					var h = img.get("height").toFloat(), w = img.get("width")
							.toFloat(), dh = this.height / h, dw = this.width
							/ w;
	...
	var Caption = new Class(
			{
				...
				initialize : function(slideshow) {
					...
					this.setOptions(options);
					var el = slideshow.el.getElement(slideshow.classes
							.get("captions")), caption = el ? el.dispose()
				...
				update : function(fast) {
					...
					if (fast) {
						...
						}).get("morph").cancel().set(
								this.classes.get("captions", p));
					} else {
						var fn1 = empty ? function() {
						} : (function(caption) {
							this.caption.store("timer", setTimeout((function(
									caption) {
								this.caption.set("html", caption)
										.morph(
												this.classes.get("captions",
														"visible"));
							}).pass(caption, this), this.caption
									.retrieve("delay")));
						}).pass(this.data.captions[this._slide], this);
						var fn2 = (function() {
							this.caption.set("aria-busy", false);
						}).bind(this);
						this.caption.set("aria-busy", true).get("morph")
								.cancel().start(
										this.classes.get("captions", "hidden"))
								.chain(fn1, fn2);
					}
	...
	var Controller = new Class(
			{
				...
				initialize : function(slideshow) {
					...
					this.setOptions(options);
					var el = slideshow.el.getElement(slideshow.classes
							.get("controller")), controller = el ? el.dispose()
							.empty() : new Element("div", {
					...
					var ul = (new Element("ul", {
						role : "menu"
					})).inject(controller), i = 0;
					Object
							.each(
									slideshow.accesskeys,
									function(accesskey, action) {
										var li = (new Element(
												"li",
												{
					...
					controller.set(
							{
								events : {
									hide : this.hide.pass(slideshow.classes
											.get("controller", "hidden"),
											controller),
									show : this.show.pass(slideshow.classes
											.get("controller", "visible"),
											controller)
								},
								morph : this.options
							}).store("hidden", false);
					slideshow.events.push("keydown",
							this.keydown.bind(slideshow)).push("keyup",
							this.keyup.bind(slideshow)).push("mousemove",
					...
					Object.each(this.accesskeys, function(accesskey, action) {
						if (e.key == accesskey.key
							...
							if (this.controller.get("aria-hidden") == "true") {
								this.controller.get("morph").set(
										this.classes.get("controller",
												"visible"));
							}
				...
				keyup : function(e) {
					Object.each(this.accesskeys, function(accesskey, action) {
						if (e.key == accesskey.key
								...
								&& e.alt == accesskey.alt) {
							if (this.controller.get("aria-hidden") == "true") {
								this.controller.set("aria-hidden", false)
										.fireEvent("hide");
							}
				...
				mousemove : function(e) {
					var images = this.el.retrieve("images").getCoordinates(), action = e.page.x > images.left
							&& e.page.x < images.right
							&& e.page.y > images.top
							&& e.page.y < images.bottom ? "show" : "hide";
	...
	var Loader = new Class(
			{
				...
				initialize : function(slideshow) {
					...
					var loader = (new Element("div", {
						...
						morph : this.options,
						role : "progressbar"
					})).store("animate", false).store("i", 0).store("delay",
							1000 / this.options.fps).inject(slideshow.el);
					slideshow.loader = loader;
					var url = loader.getStyle("backgroundImage").replace(
							/url\(['"]?(.*?)['"]?\)/, "$1").trim();
					if (url) {
						if (url.test(/\.png$/) && Browser.ie
								&& Browser.version < 7) {
							loader
									.setStyles({
						...
						new Asset.image(url, {
							onload : function() {
								var size = loader.getSize(), width = this
										.get("width"), height = this
										.get("height");
								if (width > size.x) {
					...
					loader.set("events", {
						...
						hide : this.hide.pass(slideshow.classes.get("loader",
								"hidden"), loader),
						show : this.show.pass(slideshow.classes.get("loader",
								"visible"), loader)
					});
	...
	var Thumbnails = new Class(
			{
				...
				initialize : function(slideshow) {
					...
					this.setOptions(options);
					var el = slideshow.el.getElement(slideshow.classes
							.get("thumbnails")), thumbnails = el ? el.empty()
							: new Element("div", {
					...
					thumbnails.store("props", props).store("delay",
							1000 / this.options.fps);
					slideshow.events.push("mousemove", this.mousemove
							.bind(thumbnails));
				...
				onload : function(i) {
					var thumbnails = this.thumbnails, a = thumbnails
							.getElements("a")[i];
					if (a) {
						(function(a) {
							var visible = i == this.slide ? "active"
									: "inactive";
							a.store("loaded", true).get("morph").set(
									this.classes.get("thumbnails", "hidden"))
									.start(
											this.classes.get("thumbnails",
													visible));
						}).delay(Math.max(1000 / this.data.thumbnails.length,
								100), this, a);
					}
					if (thumbnails.retrieve("limit")) {
						return;
					}
					var props = thumbnails.retrieve("props"), options = this.options.thumbnails, pos = props[1], length = props[2], width = props[4], liObj = thumbnails
							.getElement("li:nth-child(" + (i + 1) + ")");
					li = liObj.getCoordinates();
					...
					var units = Math.floor(div[width] / li[width]), x = Math
							.ceil(this.data.images.length / units), r = this.data.images.length
							% units, len = x
							* (li[length]
									+ parseFloat(liObj.getStyle("margin-"
											+ props[0])) + parseFloat(liObj
									.getStyle("margin-" + props[1]))), ul = thumbnails
							.getElement("ul").setStyle(length, len);
				...
				scroll : function(n, fast) {
					var div = this.getCoordinates(), ul = this.getElement("ul")
							.getPosition(), props = this.retrieve("props"), axis = props[3], delta, pos = props[0], size = props[2], value, tween = this
							.getElement("ul").set("tween", {
								property : pos
							}).get("tween");
					if (n != undefined) {
						var uid = this.retrieve("uid"), li = document.id(
								uid + n).getCoordinates();
						delta = div[pos] + div[size] / 2 - li[size] / 2
								- li[pos];
						value = (ul[axis] - div[pos] + delta).limit(this
								.retrieve("limit"), 0);
						tween[fast ? "set" : "start"](value);
					} else {
						var area = div[props[2]] / 3, page = this
								.retrieve("page"), velocity = -(this
								.retrieve("delay") * 0.01);
						if (page[axis] < div[pos] + area) {
							delta = (page[axis] - div[pos] - area) * velocity;
						} else if (page[axis] > div[pos] + div[size] - area) {
							delta = (page[axis] - div[pos] - div[size] + area)
									* velocity;
						}
						if (delta) {
							value = (ul[axis] - div[pos] + delta).limit(this
				...
				update : function(fast) {
					var thumbnails = this.thumbnails, uid = thumbnails
							.retrieve("uid");
					thumbnails.getElements("a").each(
							function(a, i) {
								if (a.retrieve("loaded")) {
									if (a.retrieve("uid") == this._slide) {
										if (!a.retrieve("active", false)) {
											a.store("active", true);
											var active = this.classes.get(
													"thumbnails", "active");
											if (fast) {
